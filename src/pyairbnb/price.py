...
def get(
    ...
    sections = utils.get_nested_value(
        data, "data.presentation.stayProductDetailPage.sections.sections", {}
    )
    for section in sections:
        if section["sectionId"] == "BOOK_IT_SIDEBAR":
            price_data = utils.get_nested_value(
                section, "section.structuredDisplayPrice", {}
            )
            finalData = {
                "main": {
                    "price": utils.get_nested_value(
                        price_data, "primaryLine.price", {}
                    ),
                    "discountedPrice": utils.get_nested_value(
                        price_data, "primaryLine.discountedPrice", {}
                    ),
                    "originalPrice": utils.get_nested_value(
                        price_data, "primaryLine.originalPrice", {}
                    ),
                    "qualifier": utils.get_nested_value(
                        price_data, "primaryLine.qualifier", {}
                    ),
                },
                "details": {},
            }
            details = utils.get_nested_value(
